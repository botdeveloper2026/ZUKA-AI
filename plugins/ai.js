...
zuka-ai({
    ...
    category: "ai",
    react: "🤖",
    use: ".zuka-ai <your query>",
    filename: __filename,
}, async (conn, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for Zuka Ai.\n\nExample:\n.zuka-ai What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Zuka-Ai API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/41lt2l.jpg'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Zuka Ai Response:*\n\n${gptResponse}`;
...
zuka-ai({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/41lt2l.jpg'; // Replace with a valid image URL
